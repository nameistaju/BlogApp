...
const BlogItem = ({ blog }) => {
  ...
  const contentPreview = 
    content?.length > 150 ? `${content.substring(0, 150)}...` : content || 'No content available';
  ...
  const authorName = typeof author === 'object' ? author?.name || 'Unknown author' : author || 'Unknown author';
  
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-hidden transition hover:shadow-md duration-300 flex flex-col h-full">
      ...
      <div className="p-4 flex flex-col gap-3 flex-grow">
        ...
        <p className="text-sm text-gray-600 flex-grow">{contentPreview}</p>
        ...
        <div className="border-t mt-4 pt-3 flex items-center justify-between text-sm">
          ...
          <Link
            to={`/blogs/${blogId}`}
            className="bg-blue-600 text-white text-sm px-4 py-1.5 rounded-md hover:bg-blue-700 transition whitespace-nowrap"
          >
            Read More
          </Link>
        </div>
